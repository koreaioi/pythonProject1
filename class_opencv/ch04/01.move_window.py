...
# 왜 주석처리해도 위치는 그대로인가?
# cv2.moveWindow(title1, 0,0) #윈도우 창 title1을 150,150 으로 이동
# cv2.moveWindow(title2, 400,50) #윈도우 창 title2를 400, 50 으로 이동

cv2.imshow(title1, image) #윈도우 창과 윈도우에 이미지를 보여준다.
cv2.imshow(title2, image)

cv2.waitKey(0) #입력을 기다림 0이면 계속 기다리고, 1이상의 수를 넣으면 해당수 만큼 기다림
